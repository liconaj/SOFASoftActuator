...
class Properties:
    ...
    material_parameter_set: str = "10000 120000 10e5"
    poisson_ratio: float = 0.49
    young_modulus: float = 100e3
...
def createScene(root: Sofa.Core.Node) -> Sofa.Core.Node:
    root.dt.value = 0.05
    root.gravity.value = [0, 0, -9.8]

    loadPlugins(root)

    # Visualización
    root.addObject("LineAxis", size=0.3)
    root.addObject("VisualStyle", displayFlags="hideVisual showBehavior")

    # Loop animación
    root.addObject("DefaultAnimationLoop")

    hook = createHook(root, "Hook")
    cavity_1 = createCavity(hook, "Cavity1", Models.hook_cavity_1_stl, 50000)
...
def createHook(parent: Sofa.Core.Node, name: str) -> Sofa.Core.Node:
    ...
    hook.addObject("CGLinearSolver", iterations=100, tolerance=1e-5, threshold=1e-5)
